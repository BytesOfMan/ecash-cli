...
/**
 * Print output from chronik.tokenId(tokenId).history(page, pageSize) to console
 * @param {string} tokenId valid tokenId
 * @param {number} page page to fetch
 * @param {number} pageSize number of txs per page
 */
async function printHistory(tokenId, page, pageSize) {
    let response;
    try {
        response = await chronik.tokenId(tokenId).history(page, pageSize);
    } catch (err) {
...
printHistory(tokenId, page, pageSize);
